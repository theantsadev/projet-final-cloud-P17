...
function VisitorDashboard() {
  ...
  const loadSignalements = async () => {
    try {
      ...
      // Load signalements and recap in parallel
      const [signalements, recapData] = await Promise.all([
        signalementAPI.getAll(),
        signalementAPI.getRecap()
      ])

      // Transform backend data to frontend format
      const transformedWorks = signalements.data.map(s => ({
        id: s.id,
        title: `Signalement ${s.entreprise || 'Travaux'}`,
  ...
  const stats = recap ? {
    total: recap.total || roadWorks.length,
    pending: pendingCount,
    inProgress: inProgressCount,
    completed: recap.termines || completedCount,
    totalSurface: recap.surfaceTotale || 0,
    totalBudget: recap.budgetTotal || 0,
    progress: recap.pourcentageTermines || 0
  } : {
